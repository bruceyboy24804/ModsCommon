...
namespace ModsCommon.Extensions {
    ...
    public class GenericUIReader<T> : IReader<T> {
        /// <inheritdoc/>
        private static readonly Dictionary<Type, object> Readers = (Dictionary<Type, object>)typeof(ValueReaders).GetField("s_Readers", BindingFlags.NonPublic | BindingFlags.Static).GetValue(null);
        ...
        {
            if (Readers.TryGetValue(type, out var valueReader))
            {
                return valueReader;
            }
            ...
            return Readers[type] = new GenericUIReader<T>();
        }
